...
/// Patente
///---------------
yumKaaxControllers.controller("PatenteCtrl", ["$scope", "$http", "$q", "$routeParams", "$timeout", "$route", "listService", "authService", "$location", "dateHelper",
        ...
        $http.post(authService.api() + service + "SaveSolicitud", { model: $scope.model })
            .then(
                function (result) {
                    ...
                    $scope.appPopupBox.handleError(null, 'Expediente actualizado - ' + $routeParams.tipo +'-'+ $routeParams.numero);
                },
                function (error) {
